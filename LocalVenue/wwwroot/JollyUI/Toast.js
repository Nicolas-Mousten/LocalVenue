...
const ENTER = "enter";
const LEAVE = "leave";
const ENTER_ACTIVE = "enterActive";
const ENTER_FROM = ENTER + "From";
const ENTER_TO = ENTER + "To";
const LEAVE_ACTIVE = "leaveActive";
...
const OPTION_ARIA_DESCRIBEDBY = "ariaDescribedby";
...
var camelToKebab = (str = "") =>
    cache$2[str] ||
    (cache$2[str] = str.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase());
...
var inDOM = (elem) => elem && document.body.contains(elem);
...
var kebabToCamel = (str = "") =>
    cache$1[str] ||
    (cache$1[str] = str.replace(/-./g, (match) => match[1].toUpperCase()));
...
let isSupported$1;
var isPopoverApiSupported = () =>
    isSupported$1 === undefined
        ? (isSupported$1 = HTMLElement.prototype.hasOwnProperty(POPOVER))
        : isSupported$1;

let isSupported;
var isCSSDirSupported = () =>
...
window.Toast = {
    success: function (content, time) {
        new Toast({ content, type: "success", autohide: time});
    },
    info: function (content, time) {
        new Toast({ content, type: "info", autohide: time});
    },
    error: function (content, time) {
        new Toast({ content, type: "danger", autohide: time}); // Use "danger" class for error toast
    },
    warning: function (content, time) {
        new Toast({ content, type: "warning", autohide: time});
    },
    message: function (content, time) {
        new Toast({ content, autohide: time });
    },
}
...
window.Popover = {
    ...
    create: function (target) {
        new Popover(target, {shown: false});
    },
    init: function () {
        Popover.initAll();
    },
...
window.TabList = {
    ...
    show: function (tabListId,tabId) {
        const tablist = document.getElementById(tabListId);
        Tablist.get(tablist).show(tabId);
    },
}    
