...
namespace LocalVenue.Tests.Helpers
{
    public static class HttpClientFactoryHelper
    {
        ...
        private static string MovieCreditsUriRegex = @"^https://api\.themoviedb\.org/3/movie/\d{1,19}/credits$";
        ...
        public static Mock<IHttpClientFactory> GetActorServiceMockClientFactory()
        {
            ...
            httpMessageHandler.SetupSendAsync(HttpMethod.Get, BaseAddress + LatestMovieUri)
                .ReturnsHttpResponseAsync(LatestMovieJson, HttpStatusCode.OK);

            httpMessageHandler.SetupSendAsyncRegex(HttpMethod.Get, MovieCreditsUriRegex)
            ...
            mockFactory.Setup(_ => _.CreateClient(It.Is<string>(i => i == HttpClientName))).Returns(httpClient);
