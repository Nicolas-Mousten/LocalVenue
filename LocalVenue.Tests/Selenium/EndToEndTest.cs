...
public class EndToEndTest : IDisposable {
    ...
    public async Task EndToEnd() {
        ...
        var createShowButton = wait.Until(ExpectedConditions.ElementToBeClickable(By.Id("create-show-button")));
        ...
        driver.FindElement(By.Id("show-start-time-input")).SendKeys(DateTime.Today.AddDays(1).AddHours(16).ToString(CultureInfo.CurrentCulture));
        driver.FindElement(By.Id("show-end-time-input")).Click();
        driver.FindElement(By.Id("show-end-time-input")).SendKeys(DateTime.Today.AddDays(1).AddHours(18).ToString(CultureInfo.CurrentCulture));
        ...
        var show = driver.FindElement(By.Id("home-shows-grid")).FindElement(By.CssSelector(".col:last-child"));
    
        ((IJavaScriptExecutor)driver).ExecuteScript("arguments[0].scrollIntoView(true);", show);
    
        wait.Until(webDriver => (bool)((IJavaScriptExecutor)webDriver).ExecuteScript("return arguments[0].getBoundingClientRect().top >= 0 && arguments[0].getBoundingClientRect().bottom <= window.innerHeight;", show));
        ...
        for (int i = 1; i < 5; i++)
        {
            var seatFirstRow = driver.FindElement(By.CssSelector(".seat-display")).FindElement(By.CssSelector($".seat-row:nth-child(1) > .seat:nth-child({i * 2 - 1})"));
    
            seatFirstRow.Click();
    
            var addTicketButton = driver.FindElement(By.CssSelector(".seat-display")).FindElement(By.CssSelector($".seat-row:nth-child(1) > .ui-popover:nth-child({i * 2})"))
        ...
        var deleteButton = wait.Until(ExpectedConditions.ElementExists(By.CssSelector(".column-body-item-hover:last-child .btn-danger")));
        
        ((IJavaScriptExecutor)driver).ExecuteScript("arguments[0].scrollIntoView(true);", deleteButton);
    
        wait.Until(webDriver => (bool)((IJavaScriptExecutor)webDriver).ExecuteScript("return arguments[0].getBoundingClientRect().top >= 0 && arguments[0].getBoundingClientRect().bottom <= window.innerHeight;", deleteButton));
